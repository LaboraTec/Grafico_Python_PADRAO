...
def makeFig():    #cria a função makeFig
    ...
    plt.xlabel('t(ms)')  # insere um título para o eixo y
    plt.ylabel('y(cm)')
...
while True:   #inicia um loop
    ...
    valorX = long (objeto[1])  #guarda o primeiro valor enviado via porta serial (índice 0)
    valorY =float(objeto[0]) #guarda o segundo valor enviado via porta serial (índice 1)
    ...
    if (z<=i):  #compara o nº de interações com o nºdados selecionados pelo usuário em i. Se x<=i, realiza a seguinte tarefa:
        ...
        df.to_excel('Aquisição2.xlsx', 'Sheet1', index=False)  #gera um arquivo xlsx com os valores guardados em df
